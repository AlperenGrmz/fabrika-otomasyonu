...
const CreateEmployeeDrawer = ({employeeData, onClose, open }) => {
  const {toggleSubmit, createButtonClicked, forEdit, setForEdit} = useContext(DrawerContext)
  ...
  const handleSubmit = async (values) => {
   await mutateData(forEdit ? `employeer/${employeeData.id}` : "employeer", forEdit ?"put": "post", {
    adi: values.ad,
    soyadi: values.soyad,
  ...
  return (
    <Drawer
      ...
      extra={
        <Space>
          ...
          <Button loading={formik.isSubmitting} onClick={() => {
            onClose();
            formik.submitForm();
          }} type="primary">
            Submit
    ...
    >
      ...
      <Form labelCol={{span: 3, }} wrapperCol={{span: 24 }} onFinish={formik.handleSubmit}>

        <Form.Item htmlFor="ad" label="Ad">
       <Input name="ad" type="text" onChange={formik.handleChange} value={formik.values.ad} />
        </Form.Item>
        <Form.Item htmlFor="soyad" label="Soyad">
       <Input style={{
        width: '100%'
       }} name="soyad" type="text" onChange={formik.handleChange} value={formik.values.soyad} />
        </Form.Item>
       ...
       <TextArea maxLength={200} showCount style={{
        width: '100%'
       }} name="adres" type="text" onChange={formik.handleChange} value={formik.values.adres} />
        </Form.Item>
      </Form>
      </FormikProvider>
    </Drawer>
